...
docker-php-ext-install -j$(nproc) gd mysqli pdo pdo_mysql soap exif bz2 imap gettext bcmath
...
echo "always_populate_raw_post_data = -1" > /usr/local/etc/php/conf.d/request.ini
...
chown -R www-data:www-data /var/www/.bash_history
